...
/**
 * The council in the room, drawn with CSS 3D and SVG.
 *
 * The fallback for `Room3D`, and the whole scene when WebGL is off, unavailable or has
 * lost its context. A ground plane tilted back, a round table drawn on it as a true
 * circle — the tilt is what makes it read as an ellipse — and a figure at each seat.
 *
 * It costs almost nothing: every animation is `transform` and `opacity`, so it
 * composites and never touches layout or paint, and there is no JS animation loop at
 * all. Measured with a panelist standing, the whole page spends about 70ms of main
 * thread over five seconds, most of it the session clock rather than this.
 */
